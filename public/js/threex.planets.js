...
THREEx.Planets.createPlanet	= function(options){
	var geometry	= new THREE.SphereGeometry( options.size, 32, 32 )
	var material	= new THREE.MeshPhongMaterial({
		map		: THREE.ImageUtils.loadTexture( options.color ),
		bumpMap		: THREE.ImageUtils.loadTexture( options.bump ),
		bumpScale	: 0.5,
		specularMap	: THREE.ImageUtils.loadTexture( options.specular ),
...
THREEx.Planets.createClouds	= function(options){
	...
	imageMap.addEventListener("load", function() {
		...
		imageTrans.crossOrigin = '/crossdomain.xml';
		imageTrans.src	= options.transparency;
	}, false);
	imageMap.crossOrigin = '/crossdomain.xml';
	imageMap.src	= options.color;
